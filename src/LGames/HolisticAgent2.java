...
/**
 * The HolisticAgent2 class implements the structure and behaviours of the 
 * holistic agent with a hierarchical structure in the conceptual space.
 * 
 *
 * <p>
...
public class HolisticAgent2 extends Agent{

    protected List ontology = new ArrayList();
    protected List lexicon = new ArrayList();
    private HolisticDGame2 discriminationGame = new HolisticDGame2(ontology);
     ...
     */
    public HolisticAgent2(int n,final int a,final double e,final double s,
   ...
   /**
    ...
    /**
     * prints a matrix of the usage of word-meanings.
     */
    private void printUseMatrix(){
	for (int i=0;i<ontology.size();i++){
	    for (int j=0;j<lexicon.size();j++)
		System.out.print(lUse[i][j]+" ");
	    System.out.println();
	}
    }
	...
	if (type=='u'){
		...
		for (int i=1;i<ontology.size();i++){
			...
			if (lUse[0][j]>0) product+=(double)(lUse[i][j]*lUse[i][j])/
		...
		for (int i=1;i<lexicon.size();i++){
			...
			if (lUse[j][0]>0) product+=(double)(lUse[j][i]*lUse[j][i])/
	    ...
	    //in score-based simulations all elements with a score < threshold are pruned
	    for (int i=1;i<ontology.size();i++){
		forget=true;
	...
	//if we found an utterance u ...
	if (u>0){
	    utterance=(Symbols)lexicon.get(u);
	    utteranceID=u;
	}
	else if (Math.random()<= probability && lexicon.size()<(maxNSymbols-1)){
	    //invent a new word
	    boolean exists=true;
	    while (exists){
		exists=false;
	...
	utterance = null;
	utteranceID=0;
	if (TOPIC != null)//make sure the discrimination game succeeded.
     ...
     * @param type The score-update type
     *
	...
	boolean found = false;
	double s=0.0;
	//if discr.game succeeded and an utterance is produced...
	if (TOPIC != null && u != null){
	    //search for the proper lexical entry
		...
		if ((u.getForm()).equals(((Symbols)lexicon.get(i)).getForm())){
		    ...
		    //if the entry matches the topicID and the score is non-zero,
		    //interpretation succeeds. As the hearer in an OG knows the 
		    //topic, the OG succeeds consequently too
		    if (s>0.0){
	...
	if (u != null){
		...
		if ((u.getForm()).equals(((Symbols)lexicon.get(i)).getForm())){
			...
			//that is distinctive and in the focus of attention
			if (cxt.distinctive[j] && cxt.foa[j] && cxt.getCategory(j)>=0){
			    //select if it has the highest score or, in case of a tie
			    //with a 50-50 chance
			    //topicID will be the guessed topic
			    if (utype=='s'){
		    ...
		    found = true;
		    if (topicID>0){//set for future reference
			utterance=(Symbols)lexicon.get(i);
			utteranceID=i;
			TOPIC=(Meaning)ontology.get(topicID);
		    }
	...
	if (success){
	    if (utype=='s'){//in case of score-based
	    ...
	    else{//in case of OG or GG
		if (role == 's'){
		    //update speaker (all games) and hearer (og and gg, 
		    //no lateral inhibition here)
		    lUse[topicID][0]++;
		    lUse[0][utteranceID]++;
		    lUse[topicID][utteranceID]++;
		}
		else{//update hearer selfish game
		    //increase score for all word-meaning associations in 
		    //focus of attention
		    for (int i=0;i<cxt.categoryLength();i++){
			if (cxt.distinctive[i] && cxt.foa[i] && cxt.getCategory(i)>=0){
	    ...
	    if (utype=='s') //decrease score
		lScore[topicID][utteranceID]=eta*lScore[topicID][utteranceID];
	    else{//increase denomenator of score (only OG & GG)
		lUse[topicID][0]++;
		lUse[0][utteranceID]++;
	    }
	}
	...
	for (int i=0;i<ontology.size();i++)
	    ...
	    else//lateral inhibition meanings
		lScore[i][utteranceID]=eta*lScore[i][utteranceID];

	//lateral inhibition words
	for (int i=0;i<cxt.categoryLength();i++){
	    if (cxt.distinctive[i] && cxt.foa[i] && cxt.getCategory(i)>=0){
		for (int j=0;j<lexicon.size();j++)
		    if (j!=utteranceID)
	...
	case 'o'://OG
		...
		if (utype=='s')
		    lScore[topicID][wordIndex]=0.01;//initial score
		else{
		    lUse[topicID][wordIndex]=1;//initial usage
		    lUse[topicID][0]++;
		    lUse[0][wordIndex]++;
		}
		
		if (wordIndex == lexicon.size()){
		    //we still need to add the word to lexicon. unless the lexicon is too big..
	...
	case 'g'://GG
		...
		if (cxt.distinctive[T] && cxt.foa[T]){//Adopt form
		    //learn the right topic
		    topicID=cxt.getCategory(T);
		    if (topicID>=0){//need not be
			boolean exists=false;
			//check if the word exists. If so, only adopt meaning.
	...
	case 's'://SG
	    ...
	    else if (u!=null){//adopt the word (or meanings)
		boolean exists=false;
		if (utype=='u'){
		    for (int i=0;i<lexicon.size() && !exists;i++){
			if ((u.getForm()).equals(((Symbols)lexicon.get(i)).
						 getForm())){
			    //word exists, adopt meanings
			    for (int j=0;j<cxt.categoryLength();j++){
				if (cxt.distinctive[j] && cxt.foa[j] && cxt.getCategory(j)>=0){
     ...
     * @param l hierarchical layer
     * @param n language game number
     * @param adapt wether or not the learning is on or off.
     */
    public void playDGame(char type,char uType,int l,int n,boolean adapt){
	lg=n;
	TOPIC=null;
	layer=l;
	...
	topicID=discriminationGame.playGame(cxt,topic,layer,adapt);
	if (topicID>=0){
	    TOPIC=(Meaning)ontology.get(topicID);
	...
	else if (discriminationGame.getMaxLayer()>maxLayer) maxLayer++;
	DG++;
    }
    /**
    ...
    /**
     * @return maxLayer the maximum conceptual space layer that is currently occupied
     */
    public int getMaxLayer(){
	return maxLayer;
    }
    /**
     * @return layer the conceptual space layer that has last been investigated
     */
    public int getLayer(){
	return layer;
    }
	...
	for (int i=1;i<ontology.size()-1;i++)
		...
		if (((Meaning)ontology.get(i)).getLayer()==
		    ((Meaning)ontology.get(j)).getLayer() &&
		    Utils.distance(((Meaning)ontology.get(i)).getPrototype(),
     ...
     * entire population, used to save the lexicon in of the different formats
     */
    ...
    * Function for an artificially created lexicon.
     *
     * The ontology is just an integer. Meanings are created in order not to conflict 
     * other procedures.
     */
    ...
    * Function for an artificially created lexicon.
    ...
    public double calcProductionCoherence(HolisticAgent2 hearer,int nMeanings){
	Symbols sp,he;
	int sim=0;
	for (topicID=1;topicID<ontology.size();topicID++){
	    ...
	    //System.out.println(topicID+" "+sp.getForm()+" "+(hearer.produce_utterance(0.0,'u')).getForm()+" "+sim);
	    he=hearer.produce_utterance(0.0,'u');
	    if (he!=null && (sp.getForm()).equals(he.getForm()))
		sim++;
	}
	//System.out.println(sim+" "+nMeanings+" "+topicID+" "+ontology.size());
    ...
    * Function for an artificially created lexicon.
    ...
    public double calcInterpretationCoherence(final HolisticAgent2 hearer,int nMeanings){
	String sp,he;
	int sim=0;
	for (int i=1;i<ontology.size();i++){
	    sp=bestMatch(i);
